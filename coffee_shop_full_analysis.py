"""
╔══════════════════════════════════════════════════════════════════╗
║    COFFEE SHOP SALES — COMPLETE INSIGHT ANALYSIS                ║
║    Dataset: 149,116 Transactions | Jan–Jun 2023 | 3 Stores      ║
║    Charts: Bar, Line, Scatter, Pie, Donut, Heatmap, Box,        ║
║            Area, Histogram, Poll/Survey, Bubble, Waterfall,     ║
║            Violin, Stacked Bar, Correlation Matrix & more       ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
print(f"✅ Loaded {len(df):,} rows | {df['transaction_date'].min().date()} → {df['transaction_date'].max().date()}\n")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 01: EXECUTIVE KPI DASHBOARD ────────────────────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 01 — KPI Dashboard...")
...
plt.close()
print("  ✅ chart_01_dashboard.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 02: SCATTER — Price vs Volume (Bubble) ─────────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 02 — Scatter / Bubble: Price vs Volume...")
...
# Scatter: avg price vs total orders
ax = axes[0]
...
# Bubble: price vs revenue, bubble size = orders
ax2 = axes[1]
...
plt.close()
print("  ✅ chart_02_scatter_bubble.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 03: POLL / SURVEY STYLE — Customer Preferences ─────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 03 — Poll / Survey Style Charts...")

fig, axes = plt.subplots(2, 2, figsize=(16, 10))
fig.suptitle('📊 Customer Preference Poll — What Do People Order?',
             fontsize=14, fontweight='bold', color=C['dark'])

# Poll 1: Category preference (horizontal % bar like survey results)
ax = axes[0, 0]
...
# Poll 2: Order quantity preference (like a poll result)
ax2 = axes[0, 1]
...
# Poll 3: Price sensitivity — orders by price bucket
ax3 = axes[1, 0]
...
# Poll 4: Weekend vs Weekday preference
ax4 = axes[1, 1]
...
plt.close()
print("  ✅ chart_03_poll_survey.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 04: STACKED BAR — Monthly Revenue by Category ──────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 04 — Stacked Bar + 100% Stacked...")
...
plt.close()
print("  ✅ chart_04_stacked_bar.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 05: BOX PLOTS — Revenue Distribution ───────────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 05 — Box Plots & Violin Plots...")

fig, axes = plt.subplots(2, 2, figsize=(16, 10))
fig.suptitle('Revenue Distribution Analysis — Box & Violin Plots', fontsize=14, fontweight='bold')

# Box: revenue per transaction by store
ax = axes[0, 0]
...
# Box: revenue by product category (top 5)
ax3 = axes[1, 0]
...
# Violin: revenue by hour (peak hours 7-12)
ax4 = axes[1, 1]
...
plt.close()
print("  ✅ chart_05_box_violin.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 06: HISTOGRAM — Revenue & Price Distributions ──────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 06 — Histograms & Distributions...")
...
# Hourly order count distribution
ax4 = axes[1, 1]
...
plt.close()
print("  ✅ chart_06_histograms.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 07: AREA CHART — Cumulative & Weekly Trends ────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 07 — Area Charts...")
...
# Area: daily revenue with 7-day rolling average
ax = axes[0]
...
# Stacked area: by store over time
ax2 = axes[1]
...
plt.close()
print("  ✅ chart_07_area_charts.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 08: CORRELATION MATRIX & PAIR PLOT ─────────────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 08 — Correlation Matrix...")
...
# Scatter matrix (manual — 3 key pairs)
...
# ═══════════════════════════════════════════════════════════════════
# ── CHART 09: WATERFALL — Revenue Build-up by Category ───────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 09 — Waterfall Chart...")
...
plt.close()
print("  ✅ chart_09_waterfall.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 10: MULTI-LINE — Store Revenue by Month ────────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 10 — Multi-line & Comparison Charts...")
...
# Grouped bar: orders by store per month
ax2 = axes[1]
...
plt.close()
print("  ✅ chart_10_store_comparison.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 11: FULL HEATMAP SUITE ─────────────────────────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 11 — Heatmap Suite...")

fig, axes = plt.subplots(1, 3, figsize=(20, 6))
fig.suptitle('Revenue Heatmap Analysis', fontsize=14, fontweight='bold')

# Heatmap 1: Day × Hour (orders)
...
# Heatmap 2: Month × Category (revenue)
...
# Heatmap 3: Store × Product Category
...
plt.close()
print("  ✅ chart_11_heatmaps.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 12: DONUT / PIE SUITE ──────────────────────────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 12 — Donut & Pie Suite...")
...
plt.close()
print("  ✅ chart_12_donut_pie.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 13: TOP PRODUCTS — RANKED BAR + LOLLIPOP ───────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 13 — Product Ranking Charts...")
...
# Lollipop chart: avg order value per product
ax2 = axes[1]
...
plt.close()
print("  ✅ chart_13_product_ranking.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 14: TRAFFIC DEEP DIVE ──────────────────────────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 14 — Traffic Deep Dive...")
...
# Hourly orders with color gradient
ax = axes[0, 0]
...
# Polar / radar-style plot for hourly (as line on polar)
...
plt.close()
print("  ✅ chart_14_traffic.png")


# ═══════════════════════════════════════════════════════════════════
# ── CHART 15: INSIGHT SUMMARY — INFOGRAPHIC STYLE ────────────────
# ═══════════════════════════════════════════════════════════════════
print("Generating Chart 15 — Insights Infographic...")
...
plt.savefig('chart_15_insights_infographic.png', dpi=150, bbox_inches='tight',
            facecolor='#1A0A00')
plt.close()
print("  ✅ chart_15_insights_infographic.png")
...
chart_list = [
    ...
    ("02", "Scatter & Bubble: Price vs Volume", "Scatter, Bubble, Trend Line"),
    ("03", "Poll / Survey Style Charts",        "Horizontal Bar, Grouped Bar"),
    ("04", "Stacked Bar: Monthly by Category",  "Stacked Bar, 100% Stacked"),
    ("05", "Box & Violin: Distributions",       "Box Plot, Violin Plot"),
    ("06", "Histograms & KDE",                  "Histogram, KDE Curve"),
    ("07", "Area Charts: Daily & Store",        "Area, Stacked Area"),
    ("08", "Correlation Matrix",                "Heatmap, Scatter Matrix"),
    ("09", "Waterfall: Revenue Build-up",       "Waterfall"),
    ("10", "Store Monthly Comparison",          "Multi-Line, Grouped Bar"),
    ("11", "Heatmap Suite (3 views)",           "Heatmap × 3"),
    ("12", "Donut & Pie Suite",                 "Donut × 2, Pie"),
...
print(f"\n{'#':>3}  {'Chart':<38} {'Types Used'}")
print("-" * 65)
for num, title, types in chart_list:
    print(f"  {num}  {title:<38} {types}")

print(f"\n💰 Total Revenue:    ${total_rev:,.2f}")
print(f"📈 Growth Jan→Jun:   +{growth_pct:.1f}%")
print(f"🏪 Top Store:        {df.groupby('store_location')['revenue'].sum().idxmax()}")
print(f"☕ Top Product:      {df.groupby('product_type')['revenue'].sum().idxmax()}")
print(f"⏰ Peak Hour:        10:00 AM")
print(f"📅 Busiest Day:      Friday")

print("\n📚 STUDENT EXERCISES:")
exercises = [
    "1. Modify the scatter plot to colour dots by price bucket instead of category",
    "2. Add a regression line to the monthly revenue chart and project July",
    "3. Which store has the best revenue-per-transaction? (Hint: groupby + mean)",
    "4. Build a pivot table: store_location × product_category → avg unit price",
    "5. Find the top-3 performing hours for EACH store separately",
    "6. Plot a box plot comparing revenue distributions: weekday vs weekend",
    "7. Calculate week-over-week growth and find the single best week",
    "8. Which product has the best conversion (revenue per order)?",
]
for e in exercises:
    print(f"  {e}")
